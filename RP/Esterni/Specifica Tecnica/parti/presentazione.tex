...
\subsection{Architettura generale}{
	...
	\begin{figure}
		\centering
		\includegraphics[scale=0.25]{\imgs {General}.pdf} %inserire il diagramma UML
	...
	\subsection{Servizi nodeAPI}{
	L'interazione del sistema con il database MongoDB avviene tramite un server nodeJs i cui servizi sono esposti con una interfaccia http.
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.55]{\imgs {nodeAPI}.pdf}
			\label{fig:nodeAPI}
			\caption{Servizi nodeApI}
		\end{figure}
		\begin{itemize}
			
			\item \textbf{NodeApiServer:} radice dei servizi offerti da nodeApi, ovvero servizi di autenticazione e di interazione con il database per salvare in modo persistente le presentazioni degli utenti in remoto
			
			\item \textbf{User:} servizi disponibili al path "/users", offre funzionalità di registrazione e autenticazione di un utente attraverso token scambiati dal client al server ad ogni richiesta ai servizi in "/api"

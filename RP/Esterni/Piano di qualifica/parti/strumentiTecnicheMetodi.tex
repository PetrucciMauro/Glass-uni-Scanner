...
\subsection{Strumenti}{
	\label{sec:strumenti}
	Di seguito verranno elencati gli strumenti software\ped{g} che sono o saranno utilizzati dal gruppo per effettuare le operazioni di verifica e validazione\ped{g}:
	\begin{itemize}
		\item \textbf{Aspell}: correttore ortografico per documenti redatti in \LaTeX;
		\item \textbf{Aptana}: scelto dal gruppo per la stesura del codice\ped{g} ha integrato al suo interno varie funzioni\ped{g} di debugging ed esecuzione del codice\ped{g};
		\item \textbf{W3C\ped{g} validator}: sito che controlla la validità dei markup nei documenti scritti in HTML;
		\item \textbf{Jenkins}: sistema di per l'integrazione continua del codice\ped{g} e dei file\ped{g} latex;
		\item \textbf{jSHint}: tool di supporto per la ricerca di eventuali errori nel codice\ped{g} javascript\ped{g};
		\item \textbf{Selenium IDE}: estensione\ped{g} di Firefox che permette di registrare test tramite browser\ped{g};
		\item \textbf{ApacheBrench}: strumento a linea di comando utilizzato per misurare l’efficienza di un server\ped{g} web\ped{g} ed in grado di simulare situazioni di sovraccarico della rete;
		\item \textbf{SpeedTracer}: plugin per Google Chrome che permette di verificare l’efficienza di un’applicazione web\ped{g} durante la sua esecuzione.
	\end{itemize}
	Verranno utilizzati anche tutti gli strumenti già integrati all'interno del browser\ped{g} per lo sviluppo delle pagine web\ped{g}.
	}
\subsection {Tecniche}{
	\begin{itemize}

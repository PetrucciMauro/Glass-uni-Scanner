\section{NodeServer}{
	...
	\begin{figure}[H]
		\includegraphics[scale=0.7]{\imgs {nodeAPI}.pdf}
		\label{fig:nodeAPI}
		\caption{Servizi RESTfull offerti dal Server\ped{g} nodeJs}
	\end{figure}
	
	La gestione degli errori da parte della API REST nodeApi si basa su:
	\begin{itemize}
	\item stato della risposta http del server 
	\item campo dati 'message' nel corpo della risposta
	\item campo dati 'success' nel corpo della risposta
	\end{itemize}
	
	Gli stati http usati nelle risposte del server sono:
	\begin{itemize}
	\item 500, errore del server
	\item 400, errore del client
	\item 404, risorsa non trovata o richiesta con campi dati assenti
	\item 403, richiesta con campo authorization nell'header non impostato
	\item 304, nessuna modifica effettuata
	\item 200, richiesta accettata 
	\end{itemize}
	
	Ogni risposta del server con stato diverso da 200 contiene nel campo dati "message" del corpo una descrizione verbosa dell'errore riscontrato.
	...
	\subsection{Risorse\ped{g} e Servizi}{
	...
	\begin{itemize}
		...
		\item \textbf{PresentationElement:}
			...
			\item   \textbf{POST} /private/api/presentations/[presentationName]/element
				...
				\begin{figure}[H]
				\includegraphics[scale=0.7]{\imgs {jsonPostePut}.pdf}
		...
		\item \textbf{PresentationPath:}
			...
			\item   \textbf{PUT} /private/api/presentations/[presentationName]/path
				...
				\item \textbf{input:} in header campo Authorization il token di autenticazione, nel body in formato application/json il campo: "element" come da definizione del JSON Schema seguente		
				\item \textbf{output:} body in formato application/json; success : boolean, message : string
				\end{itemize}
			\end{itemize}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[scale=0.7]{\imgs {jsonPath}.pdf}
				\label{fig:JSON Schema PATH}
				\caption{JSON Schema PATH}
				\end{figure}
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		\item \textbf{GetImage:}
			...
			\item    \textbf{GET} /files/[user]/image/[imagename]
				\begin{itemize} 
				\item \textbf{descrizione:} ritorna il File\ped{g} [imagename] nella cartella /users/[username]/image
				\item \textbf{input:} in header campo Authorization il token di autenticazione
				\item \textbf{output:} File\ped{g} statico
				\end{itemize}
			\end{itemize}
		...
		\item \textbf{GetAudio:}
			...
			\item    \textbf{GET} /files/[user]/audio/[audioname]
				\begin{itemize} 
				\item \textbf{descrizione:} ritorna il File\ped{g} [audioname] nella cartella /users/[username]/audios
				\item \textbf{input:} in header campo Authorization il token di autenticazione
				\item \textbf{output:} File\ped{g} statico
				\end{itemize}		
			\end{itemize}
		...
		\item \textbf{GetVideo:}
			...
			\item    \textbf{GET} /files/[user]/video/[videoname]
				\begin{itemize} 
				\item \textbf{descrizione:} ritorna il File\ped{g} [videoname] nella cartella /users/[username]/videos					\item \textbf{input:} in header campo Authorization il token di autenticazione
				\item \textbf{output:} File\ped{g} statico
				\end{itemize}
			\end{itemize}
		...
		\item \textbf{Image:}
			...
			\item    \textbf{POST} /private/api/files/image/[imagename]
				...
				\item \textbf{input:} in header campo Authorization il token di autenticazione; body in formato "multipart/form-data"; File\ped{g} da caricare
				\item \textbf{output:} body in formato application/json; success : boolean, message : string
				\end{itemize}
